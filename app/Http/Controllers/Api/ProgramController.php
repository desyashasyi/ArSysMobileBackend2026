...
class ProgramController extends Controller
{
            ...
            ->map(function ($event) {
                ...
                foreach ($applicants as $app) {
                    ...
                    foreach ($app->defenseExaminer as $examiner) {
                        ...
                        if (!$examiner->defenseExaminerPresence || $examiner->defenseExaminerPresence->score === null) {
                            $hasMissing = true;
                            break 2;
                        }
                    }
    ...
    public function preDefenseDetail($eventId)
    {
        ...
        $applicants = EventApplicantDefense::where('event_id', $eventId)
            ...
            ->map(function ($app) use ($eventId) {
                $supervisors = $app->research->supervisor->map(function ($sup) use ($eventId) {
                    $presence = $sup->defenseSupervisorPresence;
                    // filter by event_id if presence exists
                    $score = ($presence && $presence->event_id == $eventId) ? $presence->score : null;
                    return [
                ...
                foreach ($app->research->supervisor as $sup) {
                    ...
                    $supervisorMap[$sup->supervisor_id] = ($p && $p->event_id == $eventId) ? $p->score : null;
                }
                ...
                // Examiner missing score doesn't count if examiner is own supervisor
                $hasMissingExaminer = $examiners->contains(fn($e) => !$e['has_scored'] && !$e['is_own_supervisor']);
                $hasMissingSupervisor = $supervisors->contains('has_scored', false);
                    ...
                    foreach ($room->applicant as $app) {
                        ...
                        foreach ($room->examiner as $ex) {
                            $presence = $ex->finaldefenseExaminerPresence
                                ...
                                ->first();
                            if (!$presence || $presence->score === null || $presence->score == -1) {
                                $hasMissing = true;
                                break 3;
                            }
                        }
    ...
    public function finalDefenseRoomDetail($eventId, $roomId)
    {
        ...
        $applicants = $room->applicant->map(function ($app) use ($room) {
            ...
            // Examiner missing score doesn't count if examiner is own supervisor
            $hasMissingExaminer = $examinerScores->contains(fn($e) => !$e['has_scored'] && !$e['is_own_supervisor']);
            $hasMissingSupervisor = $supervisorScores->contains('has_scored', false);
